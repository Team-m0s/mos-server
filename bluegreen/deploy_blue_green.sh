...
if [ -z "$IS_GREEN" ]; then # blue라면
  ...
  docker-compose pull green || { echo "Failed to pull green image"; docker compose logs green; exit 1; }

  echo "2. green container up"
  docker-compose up -d green || { echo "Failed to start green container"; docker compose logs green; exit 1; }
  ...
  while true; do
    ...
    sleep 3
    REQUEST=$(curl -s http://127.0.0.1:8081) # blue로 request
